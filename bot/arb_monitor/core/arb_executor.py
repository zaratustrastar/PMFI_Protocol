    ...
    try:
        ...
        if poly_api_key and poly_api_secret and poly_api_passphrase:
            ...
            client = ClobClient(
                ...
                proxy_address=poly_proxy_address,
            )
            log(f"🔑 [POLY] Using L2-authenticated ClobClient proxy={poly_proxy_address!r}")
        else:
            client = ClobClient(
                clob_url, key=poly_private_key, chain_id=chain_id,
                proxy_address=poly_proxy_address,
            )
            log(f"🔑 [POLY] Using L1-only ClobClient proxy={poly_proxy_address!r}")
...
def _unwind_poly_leg(
    ...
    try:
        ...
        clob_url = os.environ.get("POLY_CLOB_URL", "https://clob.polymarket.com")
        chain_id = int(os.environ.get("POLY_CHAIN_ID", "137"))
        client = ClobClient(clob_url, key=poly_private_key, chain_id=chain_id, proxy_address=poly_proxy_address)
